        ...
        <div class="header-title">
          ...
          <n-space>
            <n-button
              ...
              secondary
              @click="handleRefresh"
              v-if="taskData?.status === TaskStatus.ING"
              :loading="loadingRef"
            >更新</n-button>
            <n-button size="small" secondary @click="handlePreview" type="primary">预览</n-button>
            <n-popconfirm
              @positive-click="handleStopTask"
              v-if="taskData?.status === TaskStatus.ING"
            >
              <template #trigger>
      ...
      <div class="info-card">
        ...
        <div class="info-content">
          <div>
            <n-steps>
              <n-step
                :status="[ChildTaskStatus.INIT_ERROR, ChildTaskStatus.DOWNLOAD_ERROR].includes(taskData.child?.status || 0) ? 'error' : (taskData.child?.status || 0) >= 100 ? 'process' : 'wait'"
                title="触发中"
                description="增加初始化数据"
                @click="handleChangeLog(ChildTaskStatus.INIT)"
              />
              <n-step
                :status="[ChildTaskStatus.BUILD_ERROR].includes(taskData.child?.status || 0) ? 'error' : (taskData.child?.status || 0) > 100 ? 'process' : 'wait'"
                title="构建中"
              ...
              />
              <n-step
                :status="[ChildTaskStatus.RUN_ERROR, ChildTaskStatus.STOP_ERROR].includes(taskData.child?.status || 0) ? 'error' : (taskData.child?.status || 0 )>= 110 ? 'process' : 'wait'"
                title="运行中"
              ...
              />
              <n-step
                :status="[ChildTaskStatus.BIND_ERROR, ChildTaskStatus.COPY_ERROR].includes(taskData.child?.status || 0) ? 'error' : (taskData.child?.status || 0) > 110 ? 'process' : 'wait'"
                title="部署中"
                description="正在部署中，请耐心等待"
                @click="handleChangeLog(ChildTaskStatus.BINDING)"
              />
              <n-step
                :status="[ChildTaskStatus.ERROR].includes(taskData.child?.status || 0) ? 'error' : (taskData.child?.status || 0) === 200 ? 'process' : 'wait'"
                title="已完成"
...
import { Refresh, ArrowDownCircleOutline } from '@vicons/ionicons5';

const route = useRoute()
const message = useMessage()
const loadingRef = ref(false);
const loadingLogRef = ref(false);
